...
#ifndef ZTE_CHARGER_DETAIL_CAPACITY_SQC
#define ZTE_CHARGER_DETAIL_CAPACITY_SQC
#endif
...
static void battery_chg_report_fast_capacity_work(struct work_struct *work)
{
	...
	if (batt_status == POWER_SUPPLY_STATUS_CHARGING
		...
		oem_charger_type = usb_pst->prop[USB_OEM_CHARGER_TYPE];
#ifdef ZTE_CHARGER_DETAIL_CAPACITY_SQC
		if (oem_charger_type != 7 && bcdev->fake_soc > 0) {
#else
				...
				size_t len)
{
	...
	if (pst && pst->psy) {
		...
		&& batt_status == POWER_SUPPLY_STATUS_CHARGING) {
			pr_info("fast_capacity running, not report new soc\n");
		} else {
			power_supply_changed(pst->psy);
		}
#endif
				...
				const char *buf, size_t count)
{
	...
	pr_info("battery_charging_enabled_store, prop=%d, rc=%d\n", BATT_FCC_USER, rc);
