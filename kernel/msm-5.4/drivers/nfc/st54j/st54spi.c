...
static int st54spi_parse_dt(struct device *dev, struct st54spi_data *pdata)
{
	...
	pdata->pctrl_mode_spi = pinctrl_lookup_state(pdata->pctrl, "secure_element_active");
	if (IS_ERR(pdata->pctrl_mode_spi)) {
		dev_err(dev, "%s: Unable to find secure_element_active: %d\n",
			__FILE__, PTR_ERR(pdata->pctrl_mode_spi));
		return -EPERM;
	}

	pdata->pctrl_mode_idle = pinctrl_lookup_state(pdata->pctrl, "secure_element_sleep");
	if (IS_ERR(pdata->pctrl_mode_idle)) {
		dev_err(dev, "%s: Unable to find secure_element_sleep: %d\n",
