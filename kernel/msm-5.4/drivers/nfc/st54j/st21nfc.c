...
static int zte_nfc_hw_check(struct i2c_client *client, struct st21nfc_device *st21nfc_dev)
{
	int ret = 0;
...
reset_enable_gpio:
	/* pr_err(" ### ST21NFC_SET_POLARITY_HIGH ###\n"); */
	/* st21nfc_loc_set_polaritymode(st21nfc_dev, IRQF_TRIGGER_HIGH); */
	pr_info("%s Double Pulse Request\n", __func__);
	if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
		/* pulse low for 20 millisecs */
	...
	if (ret < 14) {
		...
		pr_info("%s i2c_master_recv len:%d\n", __func__, ret);
		pr_info("%s FW version=%02x.%02x.%02x%02x\n\n", __func__, nci_reset_rsp[10],
		nci_reset_rsp[11], nci_reset_rsp[12], nci_reset_rsp[13]);
		pr_info("%s HW version=%02x.%02x\n\n", __func__, nci_reset_rsp[8], nci_reset_rsp[9]);
		pr_info("%s hardware check success\n", __func__);
		do {
			ret = i2c_master_recv(client, nci_reset_rsp, NCI_RESET_RSP_LEN);
			if ((nci_reset_rsp[0] == 0x7E) || (nci_reset_rsp[1] == 0x7E)) {
				pr_info("%s Idle data\n", __func__);
				break;
			}
			pr_info("%s Reading other msg len:%d\n", __func__, ret);
		} while (ret > 0);
		ret = 0;
		goto done;
	}
...
/* QCOM and MTK54 use standard GPIO definition */
	...
	ret = zte_nfc_hw_check(client, st21nfc_dev);
	pr_info("%s : hardware check ret %d\n", __func__, ret);
	if (ret) {
