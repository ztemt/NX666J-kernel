...
static int cypress_set_mode(int mode) {
	...
	if(ret<0) {
		...
		ret = i2c_smbus_write_byte_data(client,CYPRESS4000_TOUCHKEY_MODE, mode);
	    pr_err("[%s] Set mode=%d success!ret=0x%x\n", __func__, mode, ret);
		return mode;
	}
...
static irqreturn_t cypress_touchkey_interrupt(int irq, void *dev_id)
{
	...
	printk(">>>>>>>>>>>>cypress right val = %d, cur_value = %d, status = %d \n", val, cur_value, status);
    if(status & CYPRESS_LEFT_BIT)
    {
        input_report_key(input, KEY_F8, (cur_value & CYPRESS_LEFT_BIT)?1:0);
        input_sync(input);
    }
...
//    struct cypress_platform_data *pdata = NULL;
    ...
    if(info == NULL){
        pr_err("%s: g_cypress_touch_key_info is null\n", __func__);
        goto exit;
    }
...
//    if((client == NULL) || (pdata == NULL)){
    ...
    pr_info("%s: start check fw version\n", __func__);
...
static int cp_pinctrl_init(struct device *dev, struct cypress_platform_data *pdata)
{
	...
	if (IS_ERR_OR_NULL(pdata->pinctrl_state_active)) {
		...
		pr_err("%s Can not lookup cypress_int_active pinstate %d\n", __func__, retval);
		goto err_pinctrl_lookup;
	}

	pdata->pinctrl_state_suspend
		= pinctrl_lookup_state(pdata->ts_pinctrl, "cypress_int_suspend");
	if (IS_ERR_OR_NULL(pdata->pinctrl_state_suspend)) {
		retval = PTR_ERR(pdata->pinctrl_state_suspend);
		pr_err("%s Can not lookup cypress_int_suspend pinstate %d\n", __func__, retval);
		goto err_pinctrl_lookup;
	}
...
static int cypress_touchkey_suspend(struct device *dev)
{
    ...
    client = container_of(dev, struct i2c_client, dev);
    ret = cypress_set_mode(CYPRESS4000_SLEEP_MODE);
