...
static int cypress_set_mode_left(int mode)
{
	...
	if (ret < 0){
		...
		ret = i2c_smbus_write_byte_data(client,CYPRESS4000_TOUCHKEY_MODE, mode);
	    pr_err("[%s] Set mode=%d right success!ret=0x%x\n", __func__, mode, ret);
		return mode;
	}
...
static ssize_t touchkey_command_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int cmd = 0;
    struct i2c_client *client;
	int ret_right = 0;
...
static ssize_t touchkey_firmversion_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int version = 0;
    struct i2c_client *client;
	int ret_right = 0;
...
static ssize_t touchkey_firmversion_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int version = 0;
	struct i2c_client *client;
	int ret_right = 0;
...
static ssize_t touchkey_reglist_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    u8 regval = 0;
...
static ssize_t touchkey_signal_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    u8 raw0 = 0;
    u8 raw1 = 0;
    u8 base0 = 0;
...
static ssize_t touchkey_Lsensitivity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int level = 0;
    int r_level = 0;
    struct i2c_client *client;
...
static ssize_t touchkey_Rsensitivity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int level = 0;
    int r_level = 0;
    struct i2c_client *client;
...
static ssize_t touchkey_Lsensitivity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int level;
    int new_level;
...
static ssize_t touchkey_Rsensitivity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int level;
    int new_level;
...
//      pr_err("%s: avdd ldo state2: %d\n", __func__, regulator_is_enabled(pdata->avdd_ldo));
		...
		pr_err("%s: cpress power1_gpio to set gpio %d to %d\n", pdata->power1_gpio, pdata->power_on_flag);
        //power on 1p8
...
//      ret = regulator_enable(pdata->power_gpio);
		...
		pr_err("%s: power-gpio to set gpio %d to %d\n", pdata->power_gpio, pdata->power_on_flag);	
...
static irqreturn_t cypress_touchkey_interrupt(int irq, void *dev_id)
{
	...
	printk(">>>>>>>>>>>>cypress left val = %d, cur_value = %d, status = %d \n", val, cur_value, status);
    ...
    {
        input_report_key(input, KEY_F7, (cur_value & CYPRESS_RIGHT_BIT)?1:0);
        input_sync(input);
    }
...
static int parse_dt(struct device *dev, struct cypress_platform_data *pdata)
{
    ...
    pdata->irq_gpio = of_get_named_gpio_flags(np, "touchkey,irq-gpio", 0, NULL);
	...
	/*
    pdata->power_gpio = of_get_named_gpio_flags(np, "touchkey,power-gpio", 0, NULL);
	...
	*/
	...
	if (pdata->power1_gpio < 0) {
		printk("falied to get power1_gpio!\n");
	...
	if (retval< 0) {
		printk(" cpress failed to request power1_gpio, retval = %d\n", retval);
		//return retval;
	}
...
//    if((client == NULL) || (pdata == NULL)){
	...
	sleep = cypress_set_mode_left(CYPRESS4000_SLEEP_MODE);
exit:
...
static int cp_pinctrl_init(struct device *dev, struct cypress_platform_data *pdata)
{
	...
	pdata->pinctrl_state_active
		= pinctrl_lookup_state(pdata->ts_pinctrl, "cypress_int_active_left");
	if (IS_ERR_OR_NULL(pdata->pinctrl_state_active)) {
		retval = PTR_ERR(pdata->pinctrl_state_active);
		pr_err("%s Can not lookup cypress_int_active_left pinstate %d\n", __func__, retval);
		goto err_pinctrl_lookup;
	}

	pdata->pinctrl_state_suspend
		= pinctrl_lookup_state(pdata->ts_pinctrl, "cypress_int_suspend_left");
	if (IS_ERR_OR_NULL(pdata->pinctrl_state_suspend)) {
		retval = PTR_ERR(pdata->pinctrl_state_suspend);
		pr_err("%s Can not lookup cypress_int_suspend_left pinstate %d\n", __func__, retval);
		goto err_pinctrl_lookup;
	}
...
//    int fw_ver;
   ...
   // msleep(10000);
    ...
    {
        pdata = devm_kzalloc(&client->dev, sizeof(struct cypress_platform_data), GFP_KERNEL);
        if (!pdata)
        {
		...
		if (!ret && pdata->ts_pinctrl) {
			ret = pinctrl_select_state(
					...
					pdata->pinctrl_state_active);
			if (ret < 0) {
				pr_err("%s: Failed to select cypress_int_active pinstate %d\n", __func__, ret);
			}
		}
	...
	ret=i2c_smbus_read_byte_data(client, CYPRESS4000_TOUCHKEY_FW);
	if (ret < 0){
        printk("cypress failed CYPRESS4000_TOUCHKEY_FW = %d \n", CYPRESS4000_TOUCHKEY_FW);
    }
    printk("cypress ret = %d \n", ret);


    ret = request_threaded_irq(info->irq, NULL, cypress_touchkey_interrupt, pdata->irq_flag,client->dev.driver->name, info);
    if (ret)
    {
	...
	info->new_mode_left = CYPRESS4000_SLEEP_MODE;
...
static SIMPLE_DEV_PM_OPS(cypress_touchkey_pm_ops, cypress_touchkey_suspend, cypress_touchkey_resume);
#endif
