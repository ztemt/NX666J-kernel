...
extern int ufp_mac_init(void);
extern int goodix_ts_core_init(void);
extern int edge_suppre_init(void);
...
static void tpd_get_tp_module_name(struct tpd_classdev_t *cdev)
{
	...
	case TS_CHIP_FOCAL:
		...
		break;
	case TS_CHIP_GOODIX:
		size = ARRAY_SIZE(goodix_vendor_l);
...
static ssize_t tp_gloves_read(struct file *file,
					 char __user *buffer, size_t count, loff_t *offset)
{
	ssize_t len = 0;
	uint8_t data_buf[10] = {0};
	...
	if (cdev->get_gloves) {
		cdev->get_gloves(cdev);
	}
	pr_notice("tpd: %s val:%d.\n", __func__, cdev->gloves_enable);
	len = snprintf(data_buf, sizeof(data_buf), "%u\n", cdev->gloves_enable);
	return simple_read_from_buffer(buffer, count, offset, data_buf, len);
}

static ssize_t tp_gloves_write(struct file *file,
				const char __user *buffer, size_t len, loff_t *off)
{
	int ret = 0;
	unsigned int input = 0;
					 ...
					 char __user *buffer, size_t count, loff_t *offset)
{
	...
	if (cdev->get_edge_limit_level) {
		cdev->get_edge_limit_level(cdev);
	}
	pr_notice("tpd: %s val:%d.\n", __func__, cdev->edge_limit_level);
				...
				const char __user *buffer, size_t len, loff_t *off)
{
	...
	if (input > 10) {
		pr_notice("tpd: error! value is too big! val = %d\n", input);
		goto exit;
	}

	pr_notice("tpd: %s val = %d\n", __func__, input);

	if (cdev->set_edge_limit_level) {
		cdev->set_edge_limit_level(cdev, input);
	}
				...
				const char __user *buffer, size_t len, loff_t *off)
{
	...
	if (ret)
		return -EINVAL;
	input = input > 0 ? 1 : 0;
	pr_notice("tpd:headset_state: %s val %d.\n", __func__, input);
...
static const struct file_operations proc_ops_gloves = {
	.owner = THIS_MODULE,
	.read = tp_gloves_read,
	.write = tp_gloves_write,
};
...
static void create_tpd_proc_entry(void)
{
	...
	tpd_proc_entry = proc_create(PROC_TOUCH_GLOVES, 0664, tpd_proc_dir, &proc_ops_gloves);
	if (tpd_proc_entry == NULL)
		pr_err("proc_create gloves failed!\n");
...
void tpd_proc_deinit(void)
{
	...
	remove_proc_entry(PROC_TOUCH_GLOVES, tpd_proc_dir);
...
static int tpd_class_init(void)
{
	...
	tpd_fw_cdev.get_gloves = NULL;
	tpd_fw_cdev.set_gloves = NULL;
	...
	tpd_fw_cdev.get_edge_limit_level = NULL;
...
static int tp_module_probe(void)
{
	tpd_class_init();
	ufp_mac_init();
	edge_suppre_init();
