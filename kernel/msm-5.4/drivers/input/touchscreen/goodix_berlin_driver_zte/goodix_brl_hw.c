...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		...
		if (ret < 0) {
			...
			gpio_direction_output(cd->avdd_enable_gpio, 0);
...
int zte_brl_set_hori(struct goodix_ts_core *cd, int ori)
{
	...
	if (mutex_trylock(&ts_core_data->mutex_cmd)) {
		cmd.cmd = 0x17;
		cmd.len = 0x5;
		cmd.data[0] = ori;
...
static int brl_dev_confirm(struct goodix_ts_core *cd)
{
	...
	} else if (strncmp(fw.rom_pid, "BERLIN", 6) != 0) {
			ret = -EINVAL;
			ts_err("device confirm failed");
	}
									...
									struct ts_rawdata_info *info)
{
	...
	while (count--) {
		...
		if (retry < 0) {
			ts_err("rawdata is not ready val:0x%02x count:%d, exit!",
				val, count);
			ret = -EINVAL;
			goto exit;
		}
		...
		if (count == RAWDATA_PRINT_COUNT - 1) {
			ret = brl_read(cd, ref_addr, (u8 *)temp_buf,
					(tx + 2) * rx * sizeof(s16));
			if (ret < 0) {
				ts_err("obtain ref_data failed, exit!");
				goto exit;
			}
			goodix_rotate_abcd2cbad(tx + 2, rx, temp_buf);
			for (i = 0; i < (tx + 2) * rx; i++)
				info->buff[offset++] = temp_buf[i];

			ret = brl_read(cd, raw_addr, (u8 *)temp_buf,
					(tx + 2) * rx * sizeof(s16));
			if (ret < 0) {
				ts_err("obtain raw_data failed, exit!");
				goto exit;
			}
			goodix_rotate_abcd2cbad(tx + 2, rx, temp_buf);
			for (i = 0; i < (tx + 2) * rx; i++)
		...
		ret = brl_read(cd, diff_addr, (u8 *)temp_buf,
				(tx + 2) * rx * sizeof(s16));
		if (ret < 0) {
			ts_err("obtain diff_data failed, exit!");
			goto exit;
		}
		goodix_rotate_abcd2cbad(tx + 2, rx, temp_buf);
		for (i = 0; i < (tx + 2) * rx; i++)
