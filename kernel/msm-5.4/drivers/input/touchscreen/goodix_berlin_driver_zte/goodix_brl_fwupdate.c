...
#define TS_DEFAULT_FIRMWARE			"goodix_firmware_gt9897.bin"
 ...
 */
#define GOODIX_CFG_ID_ADDR		0x1006E
static int goodix_fw_version_compare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (fwu_ctrl->ic_config && fwu_ctrl->ic_config->len > 0) {
		...
		if (ic_cfg_id != file_cfg_id) {
			ts_info("ic_cfg_id:0x%x != file_cfg_id:0x%x", ic_cfg_id, file_cfg_id);
			return -EINVAL;
		}
...
start_update:
	...
	if (ret)
		ts_err("flash fw data enter error");
	else
