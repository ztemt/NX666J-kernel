...
static u32 map_die2pin(struct ts_test_params *test_params, u32 chn_num)
{
	...
	if (i >= test_params->max_drv_num)
		ts_err("Failed found corrresponding channel num:%d", chn_num);
	else
...
size_t goodix_strncat(char *dest, char *src, size_t dest_size)
{
	size_t dest_len = 0;

	dest_len = strnlen(dest, dest_size);
	return strlcat(&dest[dest_len], src, dest_size - dest_len - 1);
}

size_t goodix_strncatint(char *dest, int src, char *format, size_t dest_size)
{
		...
		struct file *fp)
{
	...
	if (cfg->len <= 0) {
		ts_info("Can't find valied test config");
		return 0;
	}
...
static int goodix_save_noise_data(struct goodix_ts_test *ts_test, struct file *fp)
{
	...
	for (i = 0; i < len; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,", ts_test->noisedata[0].data[i]);
		if ((i + 1) % tx == 0)
			bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "\n");
	}
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</DataContent>\n");
...
int goodix_inspect_init(void)
{
	...
	ts_info("inspect module init finish");
	return 0;
}

void goodix_inspect_exit(void)
{
	struct kobject *def_kobj = goodix_get_default_kobj();
...
/*
late_initcall(goodix_inspect_init);
module_exit(goodix_inspect_exit);
*/
MODULE_DESCRIPTION("Goodix Touchscreen inspect Module");
