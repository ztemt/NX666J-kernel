...
*/
	...
	if (QUERYBIT(gsx_gesture->gesture_type, gs_event.gesture_type)) {
		...
		if (gs_event.gesture_type == GSX_DOUBLE_TAP && cd->ztec.is_wakeup_gesture) {
			ufp_report_gesture_uevent_nubiya(cd->input_dev, DOUBLE_TAP_GESTURE);
		} else if (gs_event.gesture_type == GSX_SINGLE_TAP && cd->ztec.is_single_tap) {
