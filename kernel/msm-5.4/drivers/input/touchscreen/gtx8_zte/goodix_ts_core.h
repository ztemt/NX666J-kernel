...
{
	if (enable && !core_data->zc.is_wake_irq) {
		enable_irq_wake(core_data->irq);
		core_data->zc.is_wake_irq = true;
		ts_info("enable_irq_wake");
	} else if (!enable && core_data->zc.is_wake_irq) {
		disable_irq_wake(core_data->irq);
		core_data->zc.is_wake_irq = false;
...
static inline int goodix_do_fw_update_gt9886(int mode)
{
	return -EINVAL;
}
#else
