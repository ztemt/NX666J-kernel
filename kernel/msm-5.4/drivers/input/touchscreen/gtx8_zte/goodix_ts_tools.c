...
static bool is_tool_init = false;
static bool is_tool_exit_init = false;
...
/*
 * struct goodix_tools_dev_gt9886 - goodix tools device struct
 ...
 * @mutex: Protect goodix_tools_dev_gt9886
 */
struct goodix_tools_dev_gt9886 {
...
} *goodix_tools_dev_gt9886;


/* read data from i2c asynchronous,
 * success return bytes read, else return <= 0
 */
static int async_read(struct goodix_tools_dev_gt9886 *dev, void __user *arg)
{
	u8 *databuf = NULL;
	int ret = 0;
	u32 reg_addr, length;
 ...
 * success return bytes read, else return <= 0
 */
static int sync_read(struct goodix_tools_dev_gt9886 *dev, void __user *arg)
{
	int ret = 0;
	u8 i2c_msg_head[I2C_MSG_HEAD_LEN];
 ...
 */
static int async_write(struct goodix_tools_dev_gt9886 *dev, void __user *arg)
{
	u8 *databuf;
	int ret = 0;
	u32 reg_addr, length;
...
/**
 * goodix_tools_ioctl_gt9886 - ioctl implementation
 *
 ...
 */
static long goodix_tools_ioctl_gt9886(struct file *filp, unsigned int cmd,
					unsigned long arg)
{
	int ret = 0;
	struct goodix_tools_dev_gt9886 *dev = filp->private_data;
	...
	case GTP_ESD_ENABLE:
		if (arg == 0)
			goodix_ts_blocking_notify_gt9886(NOTIFY_ESD_OFF, NULL);
		else
			goodix_ts_blocking_notify_gt9886(NOTIFY_ESD_ON, NULL);
		break;
	case GTP_DEV_RESET:
		hw_ops->reset(ts_dev);
		break;
...
#ifdef CONFIG_COMPAT
static long goodix_tools_compat_ioctl_gt9886(struct file *file, unsigned int cmd,
				unsigned long arg)
{
...
static int goodix_tools_open_gt9886(struct inode *inode, struct file *filp)
{
	int ret = 0;

	filp->private_data = goodix_tools_dev_gt9886;
	ts_info("tools open");
	/* Only the first time open device need to register module */
	ret = goodix_register_ext_module_gt9886(&goodix_tools_dev_gt9886->module);
	if (ret) {
...
static int goodix_tools_release_gt9886(struct inode *inode, struct file *filp)
{
	int ret = 0;
	/* when the last close this dev node unregister the module */
	ret = goodix_unregister_ext_module_gt9886(&goodix_tools_dev_gt9886->module);
	return ret;
}

/**
 * goodix_tools_module_irq_gt9886 - goodix tools Irq handle
 ...
 */
static int goodix_tools_module_irq_gt9886(struct goodix_ts_core *core_data,
	struct goodix_ext_module *module)
{
	struct goodix_tools_dev_gt9886 *dev = module->priv_data;
	struct goodix_ts_device *ts_dev = dev->ts_core->ts_dev;
	const struct goodix_ts_hw_ops *hw_ops = ts_dev->hw_ops;
...
static int goodix_tools_module_init_gt9886(struct goodix_ts_core *core_data,
			struct goodix_ext_module *module)
{
	struct goodix_tools_dev_gt9886 *tools_dev = module->priv_data;
...
static const struct file_operations goodix_tools_fops = {
	.owner		= THIS_MODULE,
	.open		= goodix_tools_open_gt9886,
	.release	= goodix_tools_release_gt9886,
	.unlocked_ioctl	= goodix_tools_ioctl_gt9886,
#ifdef CONFIG_COMPAT
	.compat_ioctl = goodix_tools_compat_ioctl_gt9886,
#endif
};
...
static struct goodix_ext_module_funcs goodix_tools_module_funcs = {
	.irq_event = goodix_tools_module_irq_gt9886,
	.init = goodix_tools_module_init_gt9886,
};

/**
 * goodix_tools_init_gt9886 - init goodix tools device and register a miscdevice
 *
 * return: 0 success, else failed
 */
/*int goodix_tools_init_gt9886(void)
{
	int ret;
	if (is_tool_exit_init) {
		return 0;
	}
	goodix_tools_dev_gt9886 = kzalloc(sizeof(struct goodix_tools_dev_gt9886), GFP_KERNEL);
	if (goodix_tools_dev_gt9886 == NULL) {
		ts_err("Memory allco err");
		return -ENOMEM;
	}

	INIT_LIST_HEAD(&goodix_tools_dev_gt9886->head);
	goodix_tools_dev_gt9886->ops_mode = 0;
	goodix_tools_dev_gt9886->ops_mode |= IRQ_FALG;
	init_waitqueue_head(&goodix_tools_dev_gt9886->wq);
	mutex_init(&goodix_tools_dev_gt9886->mutex);
	atomic_set(&goodix_tools_dev_gt9886->t_count, 0);

	goodix_tools_dev_gt9886->module.funcs = &goodix_tools_module_funcs;
	goodix_tools_dev_gt9886->module.name = GOODIX_TOOLS_NAME;
	goodix_tools_dev_gt9886->module.priv_data = goodix_tools_dev_gt9886;
	goodix_tools_dev_gt9886->module.priority = EXTMOD_PRIO_DBGTOOL;
...
/*void  goodix_tools_exit_gt9886(void)
{
	is_tool_exit_init = true;
	if (!is_tool_init) {
		return;
	}
	misc_deregister(&goodix_tools_miscdev);
	kfree(goodix_tools_dev_gt9886);
	ts_info("Goodix tools miscdev exit");
}*/
/*
module_init(goodix_tools_init_gt9886);
module_exit(goodix_tools_exit_gt9886);
*/
