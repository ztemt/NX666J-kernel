...
static bool is_update_init = false;
...
static int goodix_fw_update_thread(void *data)
{
	...
	goodix_register_ext_module_gt9886(&goodix_fwu_module);
	...
	goodix_ts_blocking_notify_gt9886(NOTIFY_FWUPDATE_START, NULL);
	...
	goodix_unregister_ext_module_gt9886(&goodix_fwu_module);
out:
	...
	if (r) {
		...
		goodix_ts_blocking_notify_gt9886(NOTIFY_FWUPDATE_FAILED, NULL);
	} else {
		ts_info("fw update success");
		goodix_ts_blocking_notify_gt9886(NOTIFY_FWUPDATE_SUCCESS, NULL);
	}
	return r;
}
		...
		const char *buf, size_t count)
{
	...
	ret = goodix_do_fw_update_gt9886(mode);
	if (!ret) {
		...
		struct goodix_ext_module *module)
{
	...
	ktype = goodix_get_default_ktype_gt9886();
...
int goodix_do_fw_update_gt9886(int mode)
{
...
int goodix_fwu_module_init(void)
{
	...
	is_update_init = true;
	return goodix_register_ext_module_gt9886(&goodix_fwu_module);
}
/*
void goodix_fwu_module_exit_gt9886(void)
{
	is_exit_init = true;
	if (!is_update_init) {
		return;
...
module_exit(goodix_fwu_module_exit_gt9886);*/
