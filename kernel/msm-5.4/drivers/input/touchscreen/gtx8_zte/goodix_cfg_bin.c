...
#include "goodix_default_cfg_bin.h"

extern struct goodix_module goodix_modules_gt9886;
int goodix_ts_stage2_init_gt9886(struct goodix_ts_core *core_data);
extern int goodix_ts_core_release(void);
...
exit:
	...
	kfree(cfg_bin);
	if (r)
		goodix_ts_blocking_notify_gt9886(NOTIFY_CFG_BIN_FAILED, &r);
	else
		goodix_ts_blocking_notify_gt9886(NOTIFY_CFG_BIN_SUCCESS, &r);
...
static int goodix_read_cfg_from_headfile(struct goodix_cfg_bin *cfg_bin)
{
	int r = 0;

	cfg_bin->bin_data_len = sizeof(goodix_default_cfg_bin);
	...
	memcpy(cfg_bin->bin_data, goodix_default_cfg_bin, cfg_bin->bin_data_len);
exit:
	return r;
}
...
static int goodix_read_cfg_bin(struct device *dev, struct goodix_cfg_bin *cfg_bin)
{
	...
	if (i >= TS_RQST_FW_RETRY_TIMES) {
		...
		r = goodix_read_cfg_from_headfile(cfg_bin);
		if (r < 0)
			ts_err("goodix_read_cfg_from_headfile failed!\n");
...
static int goodix_later_init_thread(void *data)
{
	...
	if (ts_dev->cfg_bin_state == CFG_BIN_STATE_TEMP) {
		...
		ret = goodix_do_fw_update_gt9886(UPDATE_MODE_BLOCK|UPDATE_MODE_FORCE|
		...
		ret = goodix_do_fw_update_gt9886(UPDATE_MODE_BLOCK|
					  UPDATE_MODE_FLASH_CFG|
					  UPDATE_MODE_SRC_REQUEST);
		if (ret) {
	...
	ret = goodix_ts_stage2_init_gt9886(ts_core);
	if (!ret) {
