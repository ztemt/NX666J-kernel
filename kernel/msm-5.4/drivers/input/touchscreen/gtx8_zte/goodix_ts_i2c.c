		...
		struct goodix_ts_board_data *board_data)
{
	...
	prop = of_find_property(node, "goodix,edge-suppress-cmd", NULL);
	if (prop && prop->length) {
		r = of_property_read_u32_array(node, "goodix,edge-suppress-cmd",
				board_data->edge_suppress_cmd, GOODIX_MAX_CMD_NUM * GOODIX_CMD_LEN);
		if (r < 0) {
			ts_err("Unable to read goodix,edge-suppress-cmd");
			return r;
		}
	} else
		return -ENOENT;
...
static int goodix_request_handler(struct goodix_ts_device *dev)
{
	...
	case REQUEST_RELOADFW:
		...
		goodix_do_fw_update_gt9886(UPDATE_MODE_FORCE|UPDATE_MODE_SRC_REQUEST);
	break;
	case REQUEST_IDLE:
...
/*	ts_debug("pre_buf[0 2 9]0x%x, 0x%x, 0x%x", pre_buf[0], pre_buf[2], pre_buf[9]);*/

	if (event_sta == GSX_FP_TAP) {
		report_ufp_uevent(UFP_FP_DOWN);
	} else if (event_sta != GSX_FP_TAP) {
	...
	const struct i2c_device_id *dev_id)
{
	...
	ts_info("before change client addr is %d", client->addr);
	client->addr = 93;
	ts_info("after change client addr is %d", client->addr);
